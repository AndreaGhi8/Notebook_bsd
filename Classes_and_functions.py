...
class SonarDescriptorDatasetFull(Dataset):
    ...
    def gtquery(self, x, y, yaw_deg):
        ...
        yaw_diff = np.abs(self.poses[closest_index, 2] - yaw_deg)
...
def parse_real_pose3(pose):
    x, y, Y_deg = np.array(pose, copy=True)
    Y = (Y_deg+90)*np.pi/180
    Y %= 2*np.pi
    Y_deg = Y*180/np.pi
    return x, y, Y, Y_deg
...
def gtquery(td, x, y, yaw_deg):
    print("testpose:", x, y, yaw_deg)
    dist_matrix = torch.cdist(torch.Tensor([x,y]).unsqueeze(0), td.poses[:td.synth, :2].unsqueeze(0)).squeeze()  # Shape: (N, M)

    closest_index = torch.argmin(dist_matrix, dim=-1)

    print("closest_index in poses", closest_index)
    print("XY distance:", math.sqrt((x-td.poses[closest_index, 0])**2 + (y-td.poses[closest_index, 1])**2), closest_index.item())

    
    yawdiff = np.abs((yaw_deg - td.poses[closest_index, 2]) % 360)
...
def gtquery_2(self, synthpose):
    ...
    shifts = (90 - shifts) % 360

    print("shifts", shifts)
    
    yawdiff1 = np.abs(shifts - yaw_deg.item())
    yawdiff2 = (shifts - yaw_deg.item()) % 360


    a = shifts - yaw_deg.item()
    a = np.abs((a + 180) % 360 - 180)
    

    #yawdiff = np.min(np.concatenate((yawdiff1[None], yawdiff2[None]), axis=0), axis=0)
    yawdiff=a
    print("closest rotation shift:", np.argmin(yawdiff), self.angles_shifts[np.argmin(yawdiff)])
    print("yawdiff1", a)
    #print("yawdiff2", yawdiff2)

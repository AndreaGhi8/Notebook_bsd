...
class SegformerEncoder(nn.Module):
    ...
    def __init__(self, *, dims, heads, ff_expansion, reduction_ratio, num_layers, channels):
        super().__init__()
    ...
    def forward(self, x):
        return self.mit(x, return_layer_outputs=True)
...
class SegformerDecoder(nn.Module):
    ...
    def forward(self, features):
        ...
        return [out, out.clone(), out.clone(), out.clone(), out.clone()]

class Model(nn.Module):

    def __init__(self, in_channels=2, num_classes=1, img_size=256):
        super().__init__()
        self.channels = [32, 64, 160, 256]
        self.embedding_dim = 64

        self.encoder = SegformerEncoder(
            dims=tuple(self.channels),
            heads=(1, 2, 5, 8),
    ...
    def forward(self, x, reco=False):
        ...
        embed = torch.nn.functional.normalize(
            self.embed(last_feat).flatten(1), p=2, dim=1
        )
